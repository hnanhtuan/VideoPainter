...
def video_editing_prompt(prompt, instruction, llm_model, masked_image=None, target_img_caption=True):
    ...
    masked_image_caption = response.choices[0].message.content

    return prompt, masked_image_caption

def generate_video_editing_instruction(masked_image, llm_model):    
    buffered = BytesIO()
    masked_image.save(buffered, format="PNG")
    img_str = base64.b64encode(buffered.getvalue()).decode()
    ...
    if generate_type == "i2v_inpainting":
        ...
        if img_inpainting_model:
            ...
            if video_editing_instruction == 'auto':
                video_editing_instruction = generate_video_editing_instruction(masked_image, llm_model)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="The dilate size for the mask. Default is 16.",
    )
    parser.add_argument(
        "--id_adapter_resample_learnable_path",
        type=str,
